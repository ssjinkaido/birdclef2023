    ...
    "train":
        Compose([
            ...
            OneOf( [ 
                    ...
                    AddGaussianNoise(min_amplitude=0.0001, max_amplitude=0.011, p=0.5), 
                    AddGaussianSNR(min_snr_in_db=5, max_snr_in_db=15, p=0.5)
                    ],p=0.3), 
            OneOf([
                AddBackgroundNoise(sounds_path='birdclef2021-background-noise/aicrowd2020_noise_30sec/noise_30sec', min_snr_in_db=0, max_snr_in_db=3, p=0.5 ),
                AddBackgroundNoise(sounds_path='birdclef2021-background-noise/ff1010bird_nocall', min_snr_in_db=0, max_snr_in_db=3, p=0.25 ),
                AddBackgroundNoise(sounds_path='birdclef2021-background-noise/train_soundscapes', min_snr_in_db=0, max_snr_in_db=3, p=0.25 )], p=0.3),
...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        audio, orig_sr = lb.load(row.filepath, sr=None, duration=120)
        # print(audio)
        # audio, orig_sr = sf.read(row.filepath, always_2d=True, dtype="float32")
        # if orig_sr != 32000:
        #     audio = lb.resample(audio, orig_sr=orig_sr, target_sr=32000)
...
class TimmClassifier(nn.Module):
    def __init__(self, base_model_name, pretrained=True, num_classes=CFG.num_classes2, in_channels=CFG.num_channels):
        ...
        if CFG.use_drop_path:
            self.base_model = timm.create_model(
                base_model_name, pretrained=pretrained, in_chans=in_channels, drop_rate=0.4, drop_path_rate = 0.3)
        else:
            self.base_model = timm.create_model(
                base_model_name, pretrained=pretrained, in_chans=in_channels)
...
class FineTuneTimmClassifier(nn.Module):
    def __init__(self, base_model_name = CFG.model_name, num_classes=CFG.num_classes):
        ...
        checkpoint = torch.load('trainv2b1/final_pretrained_swa_tf_efficientnetv2_b1_pretrained_fold_0_0.912930.pth')
        self.backbone.load_state_dict(checkpoint['state_dict'])
        if 'v2_b0' in base_model_name or 'v2_b1' in base_model_name:
            in_features = 1280
...
if __name__ == '__main__':
    ...
    for fold in [1]:
        ...
        train_df = df_23[df_23['fold']!=fold].reset_index(drop=True)
        valid_df = df_23[df_23['fold']==fold].reset_index(drop=True)
        ...
        for epoch in range(CFG.total_epoch):
            ...
            if epoch >100:
                ...
                if val_cmap>0.808:
                    ...
                    path = f'trainv2b1/finetune_{CFG.model_name}_fold_{fold}_model_epoch_{epoch+1}_{val_cmap:.4f}.pth'
