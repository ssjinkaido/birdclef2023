...
def init_logger(log_file='swa.log'):
    ...
    def __init__(self, base_model_name, pretrained=True, num_classes=CFG.num_classes2, in_channels=CFG.num_channels):
        ...
        if CFG.use_drop_path:
            self.base_model = timm.create_model(
                base_model_name, pretrained=pretrained, in_chans=in_channels, drop_rate=0.0, drop_path_rate = 0.0)
        else:
            self.base_model = timm.create_model(
        ...
        elif 'eca_nfnet_l0' in base_model_name:
            in_features = 2304
        elif 'eca_nfnet_l1' in base_model_name:
            in_features = 3072
...
def padded_cmap_faster(solution, submission, padding_factor=5):
    solution_values = solution
    submission_values = submission
    num_cols = solution_values.shape[1]
    padded_solution_values = np.concatenate([
        solution_values,
        np.ones((padding_factor, num_cols)),
    ])
    padded_submission_values = np.concatenate([
        submission_values,
        np.ones((padding_factor, num_cols)),
    ])
    score = sklearn.metrics.average_precision_score(
        padded_solution_values,
        padded_submission_values,
        average='macro',
    )
    return score
...
out_file = 'swa_pretrained_v2b2_model_fold1_5.pth' 
iteration = [
    'trainv2b1/tf_efficientnetv2_b1_fold_0_model_epoch_90_0.91242.pth',
    'trainv2b1/tf_efficientnetv2_b1_fold_0_model_epoch_94_0.91264.pth',
    'trainv2b1/tf_efficientnetv2_b1_fold_0_model_epoch_93_0.91253.pth',
    'trainv2b1/tf_efficientnetv2_b1_fold_0_model_epoch_95_0.91246.pth',
    
    
    # 'trainv2b2/tf_efficientnetv2_b2_fold_0_model_epoch_94_0.91360.pth',
    # 'trainv2b2/tf_efficientnetv2_b2_fold_0_model_epoch_93_0.91336.pth',
    # 'trainv2b2/tf_efficientnetv2_b2_fold_0_model_epoch_89_0.91299.pth',
    # 'trainv2b2/tf_efficientnetv2_b2_fold_0_model_epoch_90_0.91294.pth'
    # 'trainv2b2/tf_efficientnetv2_b2_fold_0_model_epoch_88_0.91261.pth'
    # 'trainv2b0/tf_efficientnetv2_b0_fold_0_model_epoch_95_0.91104.pth',
    # 'trainv2b0/tf_efficientnetv2_b0_fold_0_model_epoch_94_0.91100.pth',
    # 'trainv2b0/tf_efficientnetv2_b0_fold_0_model_epoch_96_0.91057.pth',
    # 'trainv2b0/tf_efficientnetv2_b0_fold_0_model_epoch_93_0.91080.pth',
]
...
def objective(trial):
    ...
    for i in iteration:
        ...
        elif i=='trainv2b1/tf_efficientnetv2_b1_fold_0_model_epoch_95_0.91246.pth':
            LOGGER.info(f"none: {i}")
            key = list(f['state_dict'].keys())
            for k in key:
                state_dict[k] = state_dict[k] + a2*f['state_dict'][k]
        elif i=='trainv2b1/tf_efficientnetv2_b1_fold_0_model_epoch_93_0.91253.pth':
            LOGGER.info(f"none: {i}")
            key = list(f['state_dict'].keys())
            for k in key:
                state_dict[k] = state_dict[k] + a3 *f['state_dict'][k]
        elif i=='trainv2b1/tf_efficientnetv2_b1_fold_0_model_epoch_94_0.91264.pth':
            LOGGER.info(f"none: {i}")
            key = list(f['state_dict'].keys())
            for k in key:
    ...
    checkpoint = torch.load('swa_pretrained_v2b2_model_fold1_5.pth')
    model.load_state_dict(checkpoint['state_dict'])
    model.eval()
    criterion = BCEBirdLossCalculator().to(CFG.device)
    train_df = df_pre[df_pre['fold']!=0].reset_index(drop=True)
    train_df = upsample_data(train_df, thr=50)
    LOGGER.info(len(train_df))
    valid_df = df_pre[df_pre['fold']==0].reset_index(drop=True)
    LOGGER.info(len(valid_df))
    ...
    valid_targets = np.vstack(valid_targets)
    valid_predictions = np.vstack(valid_predictions)
    val_cmap = padded_cmap(pd.DataFrame(valid_targets), pd.DataFrame(valid_predictions))
    LOGGER.info(f"Val cmap: {val_cmap:.6f}")
    LOGGER.info(f"Valid loss: {loss_valid:.6f}")
    f1_score = sklearn.metrics.f1_score(valid_targets, np.array(valid_predictions)>0.3, average='micro')
    LOGGER.info(f1_score)
    if val_cmap>0.9129:
        state = {'state_dict': model.state_dict()}
        path = f'finaltrainv2b1/final_pretrained_swa_{CFG.model_name}_pretrained_fold_{fold}_{val_cmap:.6f}.pth'
